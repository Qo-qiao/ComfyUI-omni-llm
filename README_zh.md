...
2. **安装依赖**：
   - 首先安装插件依赖：
     ...
     ```
   - qwen-tts、qwen-asr、peft 三个依赖通过文件替换方式安装（不通过pip安装）：
     将准备好的三个依赖包文件分别复制到 依赖文件存放地址 E:\ComfyUI\python\Lib（确认自己的存放盘）
     [site-packages](site-packages.zip) 目录覆盖原有文件（这三个依赖文件已支持transformers-5.5.3版本）

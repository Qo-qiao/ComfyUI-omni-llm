...
NORMAL_DESCRIBE_TAGS_EN = {
    ...
    "input_template": "Analyze image content, first determine the most suitable category (choose one from: landscape, photography, illustration, IP, cosplay, game character, product, architectural interior, animal, food, UI interface, fashion, general). Then generate {mode} tag list. User input: #\n\nRules: 1. Output ONLY JSON, no extra text; 2. Total tags 30-60, no duplicates; 3. Must include quality tags (masterpiece, best quality, high resolution, ultra-detailed). 4. Strictly follow the field structure below.",
...
NORMAL_DESCRIBE_EN = {
    ...
    "description": "Automatically identify image category (landscape/photography/illustration/IP/cosplay/game character/product/architectural interior/animal/food/UI interface/fashion/general), generate structured or natural paragraph detailed description, output JSON format.",
    "input_template": "Analyze image content, first determine the most suitable category (choose one from: landscape, photography, illustration, IP, cosplay, game character, product, architectural interior, animal, food, UI interface, fashion, general). Then generate {mode} English description. User input: #\n\nDescription points: Based on category output corresponding fields, describe in detail using professional terminology.",
    "output_format_suffix": {
...
PROMPT_EXPANDER_EN = {
    ...
    "description": "Based on short prompts, automatically determine expansion type (portrait/cosplay/game character/product/scene/animal/food/general), generate detailed vivid descriptions. Preserve original intent and core keywords, output JSON format.",
    "input_template": "Expand the following prompt, first determine expansion type (portrait, cosplay, game character, product, scene, animal, food, general), then generate {mode} description. User input: #\n\nExpansion points: Add shot perspective, subject features, scene environment, action pose, lighting effects, color palette, detail rendering. Keep original meaning, enhance expressiveness. Output format see below.",
    "output_format_suffix": {
...
ZIMAGE_TURBO_EN = {
    ...
    "description": "Designed for Z-Image-Turbo model, 8-step inference for 1080P portrait generation. Automatically identifies clothing style (ancient Chinese/Japanese school/workwear/sports/wedding/street fashion/Lolita/swimwear/lingerie/cosplay/game character/general), outputs detailed descriptions for corresponding dimensions.",
    "input_template": "Optimize portrait Prompt and generate {mode} description. User input: #\n\nStep 1: Determine the clothing style category from the image/text (choose one from: Ancient Chinese/Hanfu, Japanese/School, Office/Professional, Sports/Fitness, Wedding/Formal, Street/Hip-hop, Lolita, Swimwear/Lingerie, Cosplay/Game Character, General).\nStep 2: Output corresponding field group based on category.\nMust include: camera parameters, subject appearance, pose expression, environment scene, composition rules, lighting type, color scheme.\nCategory-specific fields see format description below.",
    "output_format_suffix": {
...
WAN_T2V_EN = {
    ...
    "input_template": "Optimize video Prompt, first determine film style (choose one from: Sci-Fi, Action, Romance, Horror, Art House, Epic, Mystery, Documentary, Anime Film). Then generate {mode} description. User input: #\n\nDo not change original meaning, add cinematic aesthetics. Must include technical parameters.",
    "output_format_suffix": {

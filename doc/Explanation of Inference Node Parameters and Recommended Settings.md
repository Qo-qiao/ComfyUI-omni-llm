...
- 24GB+ VRAM: n_gpu_layers = -1, vram_limit = 24, n_batch = 4096, flash_attention = Enabled
- 16GB VRAM: n_gpu_layers = -1, vram_limit = 16, n_batch = 2048, flash_attention = Auto
- 12GB VRAM: n_gpu_layers = -1, vram_limit = 12, n_batch = 2048, flash_attention = Auto
- 8GB VRAM: n_gpu_layers = 30, vram_limit = 8, n_batch = 1024, low_vram = True
...
## 1.2 Performance Optimization Parameters Details
...
**Performance Impact**:
- Larger value means faster inference speed, but higher VRAM usage
- Smaller value means lower VRAM usage, but possibly slower inference speed
- For devices with sufficient VRAM, this value can be appropriately increased to improve performance

**Usage Recommendations**:
- 24GB+ VRAM: Can be set to 4096 or higher
- 16GB VRAM: Recommended to use default value 2048
- 8GB VRAM: Recommended to set to 1024 or lower
...
**Performance Impact**:
- Smaller value means lower VRAM usage, but may affect inference speed
- Larger value means faster inference speed, but higher VRAM usage
- Usually set to 1/4 to 1/2 of n_batch

**Usage Recommendations**:
- Keep n_ubatch at about 1/4 of n_batch
- When VRAM is insufficient, this value can be appropriately reduced

### Thread Parameters

#### n_threads (Number of Generation Threads)

**Function Description**: Number of generation threads, controlling concurrency during text generation

**Adjustment Range**: 0-64

**Recommended Setting**: 0 (auto, usually CPU cores/2)

**Performance Impact**:
- Too many threads may cause contention, actually reducing performance
- Too few threads may not fully utilize CPU performance
- Auto mode usually achieves optimal performance

**Usage Recommendations**:
- Recommended to use auto mode (0)
- If manual adjustment is needed, recommended to set to CPU cores/2

#### n_threads_batch (Number of Batch Processing Threads)

**Function Description**: Number of batch processing threads, controlling concurrency during batch processing

**Adjustment Range**: 0-64

**Recommended Setting**: 0 (auto, usually CPU cores)

**Performance Impact**:
- Affects batch processing performance
- Too many threads may cause contention
- Auto mode usually achieves optimal performance

**Usage Recommendations**:
- Recommended to use auto mode (0)
- If manual adjustment is needed, recommended to set to CPU cores

### GPU Acceleration Parameters

#### flash_attention (Flash Attention)

**Function Description**: Flash Attention acceleration technology, significantly improving inference speed
...
**Performance Impact**:
- Enabled: Significantly improves inference speed (NVIDIA GPU, usually 2-3x improvement)
- Disabled: Disables Flash Attention
- Auto: Automatically selects based on GPU (enables for NVIDIA GPU, disables for AMD GPU)

**Usage Recommendations**:
- NVIDIA GPU: Recommended to use Auto or Enabled
- AMD GPU: Use Auto (will automatically disable)
- When encountering compatibility issues, try Disabled

#### offload_kqv (Offload KQV)
...
**Performance Impact**:
- Significantly improves inference speed when enabled
- Increases VRAM usage
- For GPU mode, it is recommended to always enable

**Usage Recommendations**:
- GPU mode: Recommended to enable (True)
- CPU mode: This parameter is automatically ignored

### Memory Optimization Parameters

#### use_mmap (Memory Mapping)

**Function Description**: Use memory mapping to load model, reducing memory usage
...
**Performance Impact**:
- Reduces memory usage when enabled
- May slightly increase model loading time
- More obvious effect for large models

**Usage Recommendations**:
- When memory is sufficient: Can disable to get faster loading speed
- When memory is insufficient: Recommended to enable to reduce memory usage
- Usually keep default value True

#### use_mlock (Lock Memory)

**Function Description**: Force model to be locked in memory, preventing swapping to disk
...
**Performance Impact**:
- Improves inference performance when enabled (avoids disk swapping)
- Increases memory usage
- Requires sufficient physical memory

**Usage Recommendations**:
- When memory is sufficient: Can enable to improve performance
- When memory is insufficient: Recommended to disable
- AMD ROCm users: Recommended to enable

#### f16_kv (Half-Precision KV Cache)

**Function Description**: Use half-precision KV cache, reducing VRAM usage
...
**Performance Impact**:
- Reduces VRAM usage by about 50% when enabled
- Very small impact on generation quality
- May slightly affect inference speed

**Usage Recommendations**:
- When VRAM is insufficient: Strongly recommended to enable
- When VRAM is sufficient: Can disable to get slightly faster inference speed
- Usually keep default value True

#### low_vram (Low VRAM Mode)

**Function Description**: Low VRAM mode, sacrificing some performance to reduce VRAM usage
...
**Performance Impact**:
- Significantly reduces VRAM usage when enabled
- Reduces inference speed
- May affect generation quality

**Usage Recommendations**:
- When VRAM is insufficient: Enable this mode
- When VRAM is sufficient: Recommended to disable for optimal performance
- Use as the last optimization method
...
1. **Prioritize VRAM-related parameters**:
   - First adjust n_gpu_layers and low_vram
   - Then adjust f16_kv and n_batch
   - Finally adjust other parameters
...
5. **GPU vendor adaptation**:
   - NVIDIA GPU: Fully utilize Flash Attention
   - AMD GPU: Adjust batch size and thread count

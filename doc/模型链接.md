...
**下载链接（nsfw）**：https://huggingface.co/freeuser17/Qwen3-ASR-1.7B-Uncensored
...
### Qwen系列（建议根据clip模型选择提示词反推模型，如：Z-Image使用推荐使用qwen3VL模型生成提示词）
...
**核心特点**：Qwen3.6系列的超轻量版本，参数量最小，适合资源极其有限的设备使用，支持多模态输入和基础对话能力。
**适配场景**：极低显存设备部署、嵌入式设备、移动端应用、基础对话、简单多模态任务
...
**下载链接**：https://huggingface.co/openbmb/MiniCPM-V-4_5-gguf（含mmproj模型）


**具体型号**：MiniCPM-V-4.6
**核心特点**：国产轻量多模态旗舰，兼顾精度与速度，支持任意比例图像识别，OCR能力优秀，ComfyUI节点适配完善。
**适配场景**：提示词生成、图片/视频帧反推、多语言OCR、批量图像文本提取、日常视觉推理
**下载链接**：https://huggingface.co/mradermacher/MiniCPM-V-4.6-GGUF（含mmproj模型）


**具体型号**：MiniCPM-Llama3-V 2.5
**核心特点**：基于Llama3底座优化，轻量高效，图文融合能力提升，批量推理效率高，本地部署稳定性强。
**适配场景**：批量图片反推、快速提示词生成、轻量化OCR、低配置设备ComfyUI工作流
**下载链接**：https://huggingface.co/openbmb/MiniCPM-Llama3-V-2_5-gguf（含mmproj模型）
...
对应模型（abliterated_pe）：ministral-3-3b.safetensors
对应模型（abliterated_text_encoder）：ernie-image-prompt-enhancer.safetensors

适用生成模型：flux2-klein-9b
qwen_3_8b_fp8mixed.safetensors
https://huggingface.co/edicamargo/qwen_3_8b_fp8mixed_abliterated

适用生成模型：Z-Image、flux2-klein-4b
qwen_3_4b.safetensors
https://huggingface.co/chinmankokumin/Qwen3-4B-abliterated-v2

适用生成模型：flux2-klein-4b

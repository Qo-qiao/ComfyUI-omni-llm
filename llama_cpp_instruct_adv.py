...
class llama_cpp_instruct_adv:
    ...
    def process(self, llama_model, preset_prompt, custom_prompt, system_prompt, inference_mode, max_frames, max_size, seed, force_offload, save_states, unique_id, parameters=None, images=None, queue_handler=None):
        ...
        # Detect language based on custom_prompt and system_prompt
        # First check custom_prompt, then system_prompt if custom_prompt is empty
        input_language = self.detect_language(custom_prompt)
        if input_language == "en" and system_prompt.strip():
            # If custom_prompt is empty or English, check system_prompt language
            system_language = self.detect_language(system_prompt)
